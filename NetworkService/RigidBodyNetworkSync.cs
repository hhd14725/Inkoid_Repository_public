...
public class RigidBodyNetworkSync : MonoBehaviourPun, IPunObservable
{
    ...
    void FixedUpdate()
    {
        ...
        rb.position = Vector3.Lerp(rb.position, predictedPosition, 0.1f);

        Quaternion safeRotation = Quaternion.Normalize(networkRotation);
        rb.rotation = Quaternion.Slerp(rb.rotation, safeRotation, 0.1f);
    }
}